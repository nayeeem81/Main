...
- Update connection strings and tenant settings in appsettings.Development.json or use environment variables.
- SMTP settings used by the Worker Service are in configuration under the SmtpSettings section (Server, Port, Username, Password, SenderEmail, SenderName).

Notes

- The worker service polls the email outbox and sends messages; ensure SMTP and database configuration are available before running it.

## Architecture & Tutorial Notes

- Project structure follows a monolithic-clean approach: Web (UI), Infrastructure (DB, Email), Application (services), and Data/Entities.
- Identity is integrated and uses a shared database for identity and business entities.
- Tenant resolution is domain-based by default (local testing supported). The code includes query filters and middleware hooks to enforce tenant isolation.

Quick example — run the worker locally
...
This project is maintained by Naim (OG). It reflects years of experience and is intended as an educational, reusable template for multi-tenant SaaS stores. The design emphasizes security, tenant isolation, and practical deployment scenarios.
